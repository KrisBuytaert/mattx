...
void *thread_function(void *arg) {
    ...
    // --- GRACEFUL EXIT CHECK ---
    while (!stop_flag) {
        sleep(1); // Sleep in smaller increments to check the flag
        seconds_alive++;
        if (seconds_alive % 5 == 0) {
            printf("  [Thread %d] (TID: %d) Still alive... (%d seconds)\n", 
                   thread_id, tid, seconds_alive);
        }
    }
    
    printf("  [Thread %d] Gracefully exiting!\n", thread_id);
    return NULL; // Clean exit! No stack unwinding!
}
...
void worker_process(int num_threads) {
    ...
    while (1) {
        ...
        printf("[Worker] --- TEARING DOWN THREADS (Graceful Exit) ---\n");
        stop_flag = 1; // Tell the threads to stop!
        
        for (int i = 0; i < num_threads; i++) {
            pthread_join(threads[i], NULL); // Wait for them to return NULL
        }
        
        printf("[Worker] Threads destroyed. Sleeping 10 seconds before next wave...\n");
        stop_flag = 0; // Reset for the next wave
        sleep(10);
    }
}
